...
class PASR(nn.Module):
    ...
    def __init__(self,
        ...
        self.level3 = nn.Conv2d(
            fea_dim, fea_dim, kernel_size=(3, 3), stride=(1, 1), padding=(3, 3), dilation=(3, 3), groups=group)
        self.level4 = nn.Conv2d(
            fea_dim, fea_dim, kernel_size=(3, 3), stride=(1, 1), padding=(6, 6), dilation=(6, 6), groups=group)
        self.level5 = nn.Conv2d(
            fea_dim, fea_dim, kernel_size=(3, 3), stride=(1, 1), padding=(9, 9), dilation=(9, 9), groups=group)
        ...
        self.conv_fusion1 = nn.Conv2d(
            fea_dim * 5, fea_dim * 3, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1)
        self.conv_fusion2 = nn.Conv2d(
            fea_dim * 3, fea_dim * 1, kernel_size=(3, 3), stride=(1, 1), padding=(1, 1), groups=1)
    ...
    def feature_calc(self, x):
        ...
        if not self.use_squeeze:
            level1 = self.act(self.level1(conv_first))
            level2 = self.act(self.level2(conv_first))
            level3 = self.act(self.level3(conv_first))
            level4 = self.act(self.level4(conv_first))
            level5 = self.act(self.level5(conv_first))
        else:
            level1 = self.act(self.level1(conv_first))
            level2 = self.act(self.level2(level1))
            level3 = self.act(self.level3(level2))
            level4 = self.act(self.level4(level3))
            level5 = self.act(self.level5(level4))
...
def main():
    ...
    size = (3, 224, 224)
    img = torch.randn((1, *size))
    net = PASR(input_channels=3, output_channels=3, scale=4, num_layers=5, fea_dim=32)
    net.eval()
    flops, parms = profile(net, inputs=(img,))
    flops = flops / 1e9
    parms = parms / 1e6
    print(f"flops:{flops:.4f} G, parms:{parms:.4f} M")
    output = net(img)
    print(output.shape)
